...
| `visitors.html` | 访客页：点阵世界地图按国家着色，列出国家和地区、来源网站、页面、设备。数据是 GoatCounter 的匿名汇总，由 `.github/workflows/pages.yml` 每 3 小时用 `scripts/fetch_visitors.py` 拉取，只写进部署产物，不提交回仓库；仓库里的 `assets/visitors-data.js` 是空占位 |
| `scripts/make_world_dots.py` | 从 Natural Earth 1:110m 国界（`data/ne_110m_admin_0_countries.geojson`）重新生成 `assets/world-dots.js` |
