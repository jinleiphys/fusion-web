...
主页当前与 FUSION 主项目 2026-08-11 状态同步：20 个已交付程序技能，覆盖反应、裂变统计、R 矩阵与核天体、结构与从头算、TDHF、重离子输运，加上 SFRESCO 拟合伴生技能与 EXFOR 实验数据技能共 22 个；总语料 62,714 篇，其中 61,357 篇有全文；离线 `kb-wiki` 共 61,167 页内容（61,059 篇论文页 + 108 个主题页），连同索引与报告文件为 61,171 个 Markdown 文件。

改数字时注意：站点上的「程序技能」计数指的是驱动某个具体程序的技能（20 个），不含 EXFOR 这类研究技能；两个口径混用过一次，所以这里写明。
...
当前 FUSION 主语料：62,714 篇 arXiv nucl-th 论文，其中 61,357 篇有全文。

当前交互地图仍是上一版 61,171 篇语料的静态快照，其中 55,850 篇满足 `degree >= 2` 的布局门槛；布局使用 727,841 条语料内引用边与 PhySH 主题分类。主页已经明确标注这个版本差异，避免把地图点数误写成当前总语料规模。
